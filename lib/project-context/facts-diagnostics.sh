...
evop_render_project_context_facts_diagnostics_json() {
    printf '{"backend": %s, "lookups": %s, "hits": %s, "misses": %s, "hit_rate_percent": %s, "relative_exists_entries": %s, "file_literal_entries": %s, "file_regex_entries": %s, "file_text_entries": %s, "vscode_task_entries": %s, "command_availability_entries": %s, "command_path_entries": %s}' \
        "\"$EVOP_PROJECT_CONTEXT_FACTS_CACHE_BACKEND\"" \
        "$EVOP_PROJECT_CONTEXT_FACTS_CACHE_LOOKUPS" \
        "$EVOP_PROJECT_CONTEXT_FACTS_CACHE_HITS" \
        "$EVOP_PROJECT_CONTEXT_FACTS_CACHE_MISSES" \
