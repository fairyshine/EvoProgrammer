...
assert_contains "$inspect_output" "just [build tool]" "INSPECT should list Just as an available task-runner support tool when a Justfile exists"
assert_contains "$inspect_output" "task [build tool]" "INSPECT should list Task as an available task-runner support tool when a Taskfile exists"
...
assert_contains "$inspect_agent_output" "just -> " "INSPECT agent mode should include structured Just support entries"
assert_contains "$inspect_agent_output" "[automation; build tool; invoke declared Justfile recipes directly]" "INSPECT agent mode should describe Just support usage"
assert_contains "$inspect_agent_output" "task -> " "INSPECT agent mode should include structured Task support entries"
assert_contains "$inspect_agent_output" "[automation; build tool; invoke declared Taskfile tasks directly]" "INSPECT agent mode should describe Task support usage"
...
assert_contains "$inspect_json_summary" "task|" "INSPECT json should include structured Task support tool catalog entries"
assert_contains "$inspect_json_summary" "|build tool|automation|invoke declared Taskfile tasks directly" "INSPECT json should include Task support tool metadata"
...
assert_contains "$inspect_env_summary" $'just\tautomation' "INSPECT env should export Just support tool capability mappings"
assert_contains "$inspect_env_summary" $'task\tautomation' "INSPECT env should export Task support tool capability mappings"
...
assert_contains "$inspect_agent_env_summary" $'just\tautomation' "INSPECT agent-env should export Just support tool capability mappings"
assert_contains "$inspect_agent_env_summary" $'task\tautomation' "INSPECT agent-env should export Task support tool capability mappings"
...
printf 'just_ok=%s\n' "$([[ "$EVOP_PROJECT_CONTEXT_AGENT_COMMAND_CATALOG_RESULT" == *$'just_target\tjust --justfile ./Justfile inspect\tJustfile recipe'* ]] && printf true || printf false)"
printf 'task_ok=%s\n' "$([[ "$EVOP_PROJECT_CONTEXT_AGENT_COMMAND_CATALOG_RESULT" == *$'taskfile_target\ttask --taskfile ./Taskfile.yml verify\tTaskfile task'* ]] && printf true || printf false)"
EOF
)"
...
assert_contains "$agent_command_catalog_cache_output" "just_ok=true" "Agent command catalog should include Justfile recipes"
assert_contains "$agent_command_catalog_cache_output" "task_ok=true" "Agent command catalog should include Taskfile tasks"
...
printf 'just_ok=%s\n' "$([[ "$EVOP_PROJECT_CONTEXT_AGENT_SUPPORT_TOOLS_RESULT" == *"just [build tool]"* ]] && printf true || printf false)"
printf 'task_ok=%s\n' "$([[ "$EVOP_PROJECT_CONTEXT_AGENT_SUPPORT_TOOLS_RESULT" == *"task [build tool]"* ]] && printf true || printf false)"
EOF
)"
...
assert_contains "$agent_support_tool_cache_output" "just_ok=true" "Agent support tool discovery should include Just when a Justfile exists"
assert_contains "$agent_support_tool_cache_output" "task_ok=true" "Agent support tool discovery should include Task when a Taskfile exists"
...
printf 'just_ok=%s\n' "$([[ "$EVOP_PROJECT_CONTEXT_AGENT_SUPPORT_TOOL_CATALOG_RESULT" == *$'just\t'*$'\tbuild tool\tautomation\tinvoke declared Justfile recipes directly'* ]] && printf true || printf false)"
printf 'task_ok=%s\n' "$([[ "$EVOP_PROJECT_CONTEXT_AGENT_SUPPORT_TOOL_CATALOG_RESULT" == *$'task\t'*$'\tbuild tool\tautomation\tinvoke declared Taskfile tasks directly'* ]] && printf true || printf false)"
EOF
)"
...
assert_contains "$agent_support_tool_catalog_cache_output" "just_ok=true" "Agent support tool catalog should include structured Just entries"
assert_contains "$agent_support_tool_catalog_cache_output" "task_ok=true" "Agent support tool catalog should include structured Task entries"
